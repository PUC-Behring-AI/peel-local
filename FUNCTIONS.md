...
| `rename_clusters(cluster_dict, stem_word_frequencies, reembedded_stems=frozenset(), reembedded_source=None)` | Names a cluster by its most frequent word(s); collisions (two different clusters sharing the same dominant word -- common for a corpus-wide high-frequency term) are disambiguated with the same numeric-suffix strategy `merge_named_clusters` uses (`"trust"`, `"trust (2)"`, ...), not silently overwritten. Verified on real Boisseau data: before this fix, 6 of 20 raw noise clusters collided on `trust`/`agent`/`systems`/`expertise` and were dropped entirely (a plain dict assignment let the last-processed cluster silently erase the earlier same-named one's stems); after the fix all 20 survive under disambiguated names. Also records each cluster's own `"reembedded_stems"` (intersection of its stems with `reembedded_stems`) and a single `"reembedded_source"` (`"main"` or `"noise"`, whichever this whole call's `cluster_dict` came from -- a cluster is never a mix of the two pools) tag, used to color-differentiate the two mechanisms in the webapp/HTML export (`REEMBEDDED_SOURCE_STYLE`) | Naming/reclustering cells, `run_pipeline.py` |
...
| `recluster_large_clusters(clusters, stem_occurrences, embedder, tokenizer, model, device, ...)` | Re-splits oversized clusters using richer context. Returns `(updated_clusters, all_subclusters, reembedded_stems)` -- `reembedded_stems` is every stem actually moved into a new subcluster: its clustering embedding came from a fresh GlossBERT candidate-sense re-prediction, not the researcher's accepted definition (this function never receives `accepted_definitions`) | Reclustering cell, `run_pipeline.py` |
| `extract_noise_stems(stem_names, labels)` | Stems HDBSCAN treated as noise | Noise-rerun cell, `run_pipeline.py` |
| `recluster_noise(noise_stems, stem_occurrences, embedder, tokenizer, model, device, ...)` | Re-clusters noise stems in case some form a valid smaller group. Also never receives `accepted_definitions` -- its clustering text always comes from a fresh GlossBERT re-prediction, so every stem in its output counts as "reembedded" (callers derive this by flattening the result, e.g. `{s for stems in noise_clusters.values() for s in stems}`) | Same |
...
| `print_cluster_for_review(cluster_name, cluster_data)` | Console printer for one cluster; marks each stem in `cluster_data["reembedded_stems"]` with a `[RE-EMBEDDED -- see note below]` suffix plus an explanatory note, so a CLI/notebook reviewer sees the same disclosure the webapp shows visually | `run_cluster_review` |
...
| Function | Purpose | Called from |
|---|---|---|
| `token_forms(doc, stemmer)` | Returns `(stems, lemmas)`, one stemmed and one lemmatized string per alpha, NON-STOPWORD token (empty string at both positions for a non-alpha or stopword token) -- matches Phase 1's own ngram-mining filter (`tokenize_many_for_analysis`), not just alpha-only as before this fix. Still one entry per real document token (not compacted), so callers needing to highlight a match keep real token indices | Every other function in this file, `collocations.py`, `distant_reading.py` |
| `_compact_lemma_positions(lemmas)` | `(real_index, lemma)` for every non-empty lemma slot, skipping the gaps `token_forms` leaves at non-alpha/stopword positions -- lets n-gram matching find a contiguous run of CONTENT words regardless of what filler sits between them in the real text | `term_match_spans` |
| `term_match_spans(stems, lemmas, term)` | Each match's full tuple of REAL token indices -- length 1 for a stem, length `len(words)` for an n-gram, via `_compact_lemma_positions` for the n-gram case so a stopword/punctuation gap between an n-gram's content words (e.g. "Expertise, opacity, and trust") doesn't break the match. Needed for highlighting, where a naive contiguous range would mark the wrong tokens (or miss the later word entirely if the real gap is wider than the n-gram) | `build_reader_html` |
| `term_positions(stems, lemmas, term)` | One anchor position per match (the first matched word's real token index) -- `[span[0] for span in term_match_spans(...)]`, same contract as before this fix, just now finding matches a literal-contiguity check missed (verified on real Boisseau data: "artificial system kind"/"expert competence"/"expertise opacity trust" all went from 0 real source occurrences to their true count) | Same callers |
...
| Function | Purpose | Called from |
|---|---|---|
| `build_reader_html(doc, clusterdefs, colors_by_cluster, stemmer, ...)` | Full text with cluster terms highlighted, token-level matching. A stem/n-gram claimed by more than one cluster (a real possibility -- see recluster_noise) is highlighted with every claiming cluster's color, striped together (`_mark_background`), tooltip listing every cluster name -- not silently attributed to whichever cluster comes first alphabetically. Source-only -- the only Distant Reading analysis NOT run per-document (see `build_side_by_side_html`) | `phase3/pipeline.py::build_phase3_report` |
| `build_side_by_side_html(items, min_width=)` | Lays out `[(label, inner_html), ...]` in a wrapping flex row, one column per item -- a single item is returned unwrapped, so a corpus with no approved condensation yet renders exactly as it did before this existed | `build_phase3_report` |
| `build_wordcloud_html(text, stopwords, ...)` | Word cloud PNG (via `wordcloud`), embedded as a base64 `<img>`. Called once per document (Source + every approved condensation rate) and combined via `build_side_by_side_html` -- previously source-only, plus a since-removed duplicate per-rate rendering inside `comparison.py`'s own section | `build_phase3_report` |
...
| `build_trend_chart_svg(bin_freqs, colors_by_cluster, ...)` | Hand-rolled inline SVG line chart, no dependency. Called once per document (Source + every approved rate), combined via `build_side_by_side_html` | `build_phase3_report` |
| `build_phrase_table(doc, stopwords, min_n=, max_n=, top_n=)` | N-gram frequency table, overlap-filtered. Called once per document, combined via `build_side_by_side_html` -- degrades to a "No recurring phrases found" message rather than erroring on a short condensation with too little repeated text | `build_phase3_report` |
| `build_term_stats_table(doc, clusterdefs, stemmer, n_bins=, top_n=)` | Raw/relative frequency + peakedness (`scipy.stats.kurtosis`) + skewness (`scipy.stats.skew`) per term, with a sparkline. Excludes `raw_freq <= 1`, not just 0 -- a single occurrence is a degenerate one-hot distribution across `n_bins`, so its shape stats come out identical for every freq-1 term regardless of which bin it landed in, carrying no real signal. Called once per document, combined via `build_side_by_side_html` -- note the shape stats get noisier (not wrong) on a much shorter condensation, since there are fewer occurrences per term to bin | `build_phase3_report` |
...
| `build_comparison_section(source_doc, rate_docs, phase1_state, source_text, stemmer, stopwords, selected_pairs)` | Assembles the whole "Source vs. Summary" section -- document profile, cluster coverage (reuses `condense.compute_cluster_coverage`/`condensation_report.build_coverage_table`), Contexts, Collocates. No longer includes its own word clouds -- those now appear earlier, in the Distant Reading section's side-by-side comparison, not duplicated here | `phase3/pipeline.py::build_phase3_report` |
